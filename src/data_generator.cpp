...
auto generate_random_string(int max_string_length) -> std::string {
  ...
  std::generate_n(std::begin(result), str_length,
...
int main(int argc, char const *argv[]) {
  ...
  const std::size_t max_unique_words_count =
      (argc == 3) ? std::stoul(argv[2]) : 20'000;
  ...
  std::vector<std::string> generated_unique_words_vec{
      generated_unique_words.begin(), generated_unique_words.end()};
...

  {
    ...
    while (total_size < file_size_in_bytes) {
      auto random_index = generate_random_int(
          0, static_cast<int>(generated_unique_words_vec.size()) - 1);
  ...
  std::filesystem::rename(std::filesystem::current_path() / "tmp.txt",
                          std::filesystem::current_path() / ss.str());
